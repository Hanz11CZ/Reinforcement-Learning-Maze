...
function whatActionToTake(state,greed_parameter) {
    console.log("in whatActionToTake");
...
function updateMyQValueAndReturnIt(state,learning_rate,discount_rate,greed_parameter,goal_reward_amount) {
    ...
    if(exploitOrExplore(greed_parameter) == "explore") {
        ...
        Qvalues[state[0]+""+state[1]][action_to_take] = ((1-learning_rate)*Qvalues[state[0]+""+state[1]][action_to_take]) + (learning_rate*(next_reward+(discount_rate*updateMyQValueAndReturnIt(next_state,learning_rate,discount_rate,greed_parameter,goal_reward_amount))));
        return(Qvalues[state[0]+""+state[1]][action_to_take]);
    } else {
...
function go(startingState,learning_rate,discount_rate,greed_parameter,goal_reward_amount) {
    ...
    console.log("Win!!...final Q-values after this episode here: ");
    console.log(Qvalues);
    return;
}
...
result_buff = createInitialBoard(10,10,7,5,10);
